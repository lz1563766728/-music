...
<template>
  <div class="newsong" ref="newsong">
      <div class="listcontent" ref="content">
          <div class="listitem" v-for="item in newSongList" :key="item.id" ref="listitem" >
  ...
  methods: {
      initTabScroll(){
          ...
          for(let i=0;i<this.newSongList.length;i++){
              width += this.$refs.listitem[0].getBoundingClientRect().width+8;
              //getBoundingCLientRectRect()返回元素的大小及其相对于视口的位置  8是margin         
          }
